...
def intro(request):
    ...
    if request.GET.has_key('continue'):
        ...
        skipped_survey = SurveyStatus.objects.get_or_create(survey_id=interview_id)
        #Save skip status here
        skipped_survey[0].user_type=interview_id[0]
        skipped_survey[0].user_id=interview_id[1:7]
        skipped_survey[0].month=interview_id[-2:]
